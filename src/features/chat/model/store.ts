...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  handleChatInput: (input: string, sender: string = DEFAULT_PLAYER_NAME) => {
      ...
      case 'simpleCommand':
        switch (command.command) {
          case 'clear':
            ...
            break;
          case 'help':
            currentSendMessage("Comandos disponíveis: /clear, /help, /whisper <alvo> <mensagem>", "Sistema");
            break;
        }
        break;
      case 'textArgumentCommand':
        const whisperCommand = command;
        currentSendMessage(`[Sussurro para ${whisperCommand.target}]: ${whisperCommand.message}`, sender);
        break;
      default:
  ...
  rollAndSendMessage: (formula: string, sender: string = DEFAULT_PLAYER_NAME) => {
    try {
