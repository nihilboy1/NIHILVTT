...
interface PlayerSheetContextType {
  ...
  setSkillProficiencies: React.Dispatch<
    React.SetStateAction<NonNullable<PlayerCharacter["proficiencies"]>["skills"]>
  >;
...
export function PlayerSheetProvider({
  ...
  const [editingHitDiceEntries, setEditingHitDiceEntries] = useState<
    HitDiceEntry[]
  >(
    initialCharacter.hitDiceEntries && initialCharacter.hitDiceEntries.length > 0
  ...
  const [actions, setActions] = useState<NonNullable<PlayerCharacter["actions"]>>(
    initialCharacter.actions || []
  );
  const [attacks, setAttacks] = useState<NonNullable<PlayerCharacter["attacks"]>>(
