...
const PlayerAttributesAndSkills: React.FC<PrincipalAttributesAndSkillsProps> = ({
  attributes,
  setAttributes,
  ...
  return (
    <div className="col-span-1 flex flex-col space-y-2 border  p-2 rounded-md">
      {(Object.keys(attributes) as Array<keyof typeof attributes>).map(
        (attrName) => {
          const attrLabel = ATTRIBUTE_LABELS[attrName as keyof typeof ATTRIBUTE_LABELS]; // Usar ATTRIBUTE_LABELS
              ...
              <div className="grid grid-cols-3 items-center gap-x-1.5">
                ...
                <div className="flex flex-col items-center col-span-1">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setSavingThrowProficiencies((prev: NonNullable<PlayerToken["proficiencies"]>["savingThrows"]) => ({
                        ...prev,
                        [attrName]: e.target.checked,
                      }))
                    }
                    className={cn(
                      "h-3.5 w-3.5 rounded-sm border-surface-2 text-accent-primary focus:ring-accent-primary bg-surface-1",
                      "h-4 w-4"
                    )}
                  />
                </div>
              ...
              {parentAttributeSkills.length > 0 && (
                <div className="mt-1.5 pt-1.5 border-t border-text-secondary space-y-0.5">
                  {parentAttributeSkills.map((skillInfo) => {
                    const skillKey =
                      skillInfo.key as keyof NonNullable<PlayerToken["proficiencies"]>["skills"];
                    ...
                    return (
                      ...
                      >
                        <input
                          type="checkbox"
                          id={`skill-prof-${String(skillKey)}-${String(attrName)}`} // Adicionar attrName para ID único
                          checked={isProficient}
                          onChange={(e) =>
                            setSkillProficiencies((prev: NonNullable<PlayerToken["proficiencies"]>["skills"]) => ({
                              ...prev,
                          ...
                          className={cn(
                            "h-3.5 w-3.5 rounded-sm border-surface-2 text-accent-primary focus:ring-accent-primary bg-surface-1",
                            "h-3.5 w-3.5 col-span-1 justify-self-center"
                          )}
                        />
                        <label
                          htmlFor={`skill-prof-${String(skillKey)}-${String(attrName)}`}
                          className={cn(
                            "block text-[11px] font-medium text-accent-primary mb-px",
                            "col-span-1 text-[10px] text-center mb-0"
                          )}
                        >
                        ...
                        <label
                          ...
                          className={cn(
                            "block text-[11px] font-medium text-accent-primary mb-px",
                            "col-span-10 text-xs mb-0 cursor-pointer"
                          )}
                        >
                          {skillInfo.label}{" "}
                        </label>
