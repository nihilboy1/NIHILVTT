...
import { type Token as TokenInfo, Tool, type PageSettings, type GridInstance, type Point } from '../../types/index'; // Caminho corrigido
...
interface BoardTokenProps {
  ...
  onGridInstanceSelectForHPModal?: (instanceId: string, tokenScreenRect: DOMRect | null) => void;
...
const BoardToken: React.FC<BoardTokenProps> = ({ 
    gridInstance,
    tokenInfo, 
    cellSize, 
    zoomLevel, 
  ...
  const handleSelectThisInstance = useCallback((instanceId: string) => {
    if (onGridInstanceSelectForHPModal && tokenGroupRef.current) {
      ...
      const plainRect = screenRect ? {
         ...
         top: screenRect.top, right: screenRect.right, 
         bottom: screenRect.bottom, left: screenRect.left,
         toJSON: () => ({}) 
      } as DOMRect : null;
  ...
  const strokeWidth = isMultiSelected ? baseStrokeWidth + (Math.max(0.3, 1 / zoomLevel)) : baseStrokeWidth;
  const strokeColor = isMultiSelected 
    ? 'var(--color-accent-positive)' 
    : (isDragging ? 'var(--color-accent-primary)' : 'var(--color-border-inactive)');
  ...
  const cursorStyle = activeTool === Tool.SELECT ? (isDragging ? 'grabbing' : 'grab') : 'default';
  ...
  const firstName = useMemo(() => getFirstName(tokenInfo.name), [tokenInfo.name]);
  ...
  return (
    ...
    >
      ...
      <text
        ...
        y={nameplateYOffset} /* nameplateYOffset já é negativo, posicionando acima */
...
export default React.memo(BoardToken);
