...
import { RulerPopover } from './RulerPopover';
import { RulerPlacementMode } from '../../types/index';
...
describe('RulerPopover', () => {
  ...
  test('não deve renderizar o popover quando isOpen é false', () => {
    ...
    expect(screen.queryByRole('dialog', { name: 'Opções de Posicionamento da Régua' })).not.toBeInTheDocument();
  });

  test('deve renderizar o popover e botões quando isOpen é true e modo SNAP_TO_CENTER', () => {
    render(<RulerPopover {...defaultProps} currentMode={RulerPlacementMode.SNAP_TO_CENTER} />);
    const popover = screen.getByRole('dialog', { name: 'Opções de Posicionamento da Régua' });
    expect(popover).toBeInTheDocument();
    expect(popover).toHaveAttribute('aria-label', 'Opções de Posicionamento da Régua');

    const snapToCenterButton = screen.getByRole('option', { name: 'Fixar no Centro da Grade' });
    const freePlacementButton = screen.getByRole('option', { name: 'Posicionamento Livre' });
  ...
  test('deve renderizar o popover e botões quando isOpen é true e modo FREE_PLACEMENT', () => {
    render(<RulerPopover {...defaultProps} currentMode={RulerPlacementMode.FREE_PLACEMENT} />);
    const snapToCenterButton = screen.getByRole('option', { name: 'Fixar no Centro da Grade' });
    const freePlacementButton = screen.getByRole('option', { name: 'Posicionamento Livre' });
  ...
  test('deve chamar onSetMode com SNAP_TO_CENTER e onClose ao clicar no botão', () => {
    ...
    expect(mockOnSetMode).toHaveBeenCalledWith(RulerPlacementMode.SNAP_TO_CENTER);
  ...
  test('deve chamar onSetMode com FREE_PLACEMENT e onClose ao clicar no botão', () => {
    ...
    expect(mockOnSetMode).toHaveBeenCalledTimes(1);
    expect(mockOnSetMode).toHaveBeenCalledWith(RulerPlacementMode.FREE_PLACEMENT);
    expect(mockOnClose).toHaveBeenCalledTimes(1);
  });
