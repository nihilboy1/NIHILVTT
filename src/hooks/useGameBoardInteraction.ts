...
import { HP_MODAL_ESTIMATED_HEIGHT_REM } from "../components/modals/HPControlModal";
...
import {
  ...
  type Character, // Renomeado de TokenInfo
...
export const useGameBoardInteraction = ({
  ...
  const [preDragHPModalTokenId, setPreDragHPModalTokenId] = useState< // Renomeado
    string | null
  >(null);
  const [multiSelectedTokenIds, setMultiSelectedTokenIds] = useState< // Renomeado
    string[]
  >([]);
  ...
  const handleTokenSelectForHPModal = useCallback( // Renomeado
    (tokenId: string, tokenScreenRect: DOMRect | null) => { // Renomeado
  ...
  const handleHPChangeFromModal = useCallback(
    (tokenId: string, newHP: number) => { // Renomeado
      const token = tokensOnBoard.find( // Renomeado
        (t: Token) => t.id === tokenId // Renomeado
      );
      if (token) {
        const character = characters.find( // Renomeado
  ...
  const handleRemoveInstanceFromBoard = useCallback(
    (tokenId: string) => { // Renomeado
      ...
      setMultiSelectedTokenIds((prev: string[]) => // Renomeado
        prev.filter((id: string) => id !== tokenId) // Renomeado
      );
      if (selectedTokenId === tokenId) { // Renomeado
        setSelectedTokenId(null); // Renomeado
      }
    },
    [
  ...
  const handleHPModalAnchorShouldUpdate = useCallback(
    (_tokenId: string, newScreenRect: DOMRect | null) => { // Renomeado
  ...
  const handleTokenDragStart = useCallback( // Renomeado
    (tokenId: string) => { // Renomeado
      if (
        activeModalName === "hpControl" &&
        activeModalProps?.tokenId === tokenId // Renomeado
  ...
  const handleTokenDragMove = useCallback( // Renomeado
    (tokenId: string, visualSVGPoint: AppPoint) => { // Renomeado
      setDraggingVisuals((prev: DraggingVisuals) => {
        if (prev.tokenId === tokenId) { // Renomeado
  ...
  const handleTokenDragEnd = useCallback( // Renomeado
    (tokenId: string) => { // Renomeado
      const tokenExists = tokensOnBoard.find( // Renomeado
        (t: Token) => t.id === tokenId // Renomeado
      );
      if (tokenExists && preDragHPModalTokenId === tokenId) { // Renomeado
  ...
  const handleSetMultiSelectedTokenIds = useCallback( // Renomeado
    (ids: string[]) => {
  ...
  useEffect(() => {
    ...
    setMultiSelectedTokenIds((prev: string[]) => // Renomeado
      prev.filter((id: string) =>
        tokensOnBoard.some((t: Token) => t.id === id) // Renomeado
      )
    );
    if (
      selectedTokenId && // Renomeado
      !tokensOnBoard.some( // Renomeado
  ...
  useEffect(() => {
    if (activeModalName === "hpControl" && activeModalProps?.tokenId) { // Renomeado
      const selectedToken = tokensOnBoard.find( // Renomeado
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "Delete") {
        if (selectedTokenId) { // Renomeado
          removeToken(selectedTokenId); // Renomeado
          setSelectedTokenId(null); // Renomeado
          closeModal();
        } else if (multiSelectedTokenIds.length > 0) { // Renomeado
          multiSelectedTokenIds.forEach((id: string) => // Renomeado
