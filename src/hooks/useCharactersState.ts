...
import {
  DEFAULT_CHARACTER_DATA, // Renomeado
  DEFAULT_TOKEN_IMAGE,
...
import {
  ...
  type Character, // A nova interface Character (ficha de dados)
  type Point,
...
export interface CharactersState {
  ...
  addToken: ( // Adiciona uma instância de Token no tabuleiro
    characterId: string,
  ...
  updateTokenPosition: ( // Renomeado de updateGridInstancePosition
    tokenId: string,
    newPosition: Point
  ) => void;
...
export const useCharactersState = (): CharactersState => {
  ...
  const addCharacter = useCallback((characterData: Omit<Character, "id">): Character => {
    ...
    if (characterData.type === CharacterType.PLAYER) {
      ...
      newCharacter = {
        ...
        challengeRating: (characterData as MonsterNPCCharacter).challengeRating ?? 0,
      ...
      newCharacter = {
        ...
        maxHp: characterData.maxHp ?? DEFAULT_CHARACTER_DATA.maxHp,
        notes: characterData.notes ?? DEFAULT_CHARACTER_DATA.notes,
        isInteractive: (characterData as ObjectCharacter).isInteractive ?? false,
      } as ObjectCharacter;
    } else {
      newCharacter = {
        id: newId,
  ...
  const duplicateCharacter = useCallback(
    (characterId: string): Character | null => {
      const originalCharacter = characters.find((char) => char.id === characterId);
  ...
  const makeTokenIndependent = useCallback(
    (tokenId: string): Character | null => {
      const targetToken = tokensOnBoard.find(
        (inst) => inst.id === tokenId
      );
      if (!targetToken) {
      ...
      setCharacters((prevCharacters) => [...prevCharacters, newIndependentCharacter]);
