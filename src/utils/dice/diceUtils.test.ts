...
describe('rollDiceInternal', () => {
  ...
  test('deve rolar múltiplos dados (ex: "3d8")', () => {
    jest.spyOn(Math, 'random').mockImplementation(mockMathRandom([0.1, 0.5, 0.9])); // Simula 1, 4, 7 em 3d8
  ...
  test('deve rolar com modificador negativo (ex: "4d4-2")', () => {
    jest.spyOn(Math, 'random').mockImplementation(mockMathRandom([0.2, 0.4, 0.6, 0.8])); // Simula 1, 2, 3, 4 em 4d4
  ...
  test('deve retornar erro para número de dados zero (ex: "0d6")', () => {
    ...
    expect(result).toEqual({ error: 'Parâmetros de dados inválidos (1-100 permitidos para X e Y): 0d6' });
  });

  test('deve retornar erro para número de dados muito grande (ex: "101d6")', () => {
    const result = rollDiceInternal('101d6');
    expect(result).toEqual({ error: 'Parâmetros de dados inválidos (1-100 permitidos para X e Y): 101d6' });
  });

  test('deve retornar erro para número de lados zero (ex: "1d0")', () => {
    const result = rollDiceInternal('1d0');
    expect(result).toEqual({ error: 'Parâmetros de dados inválidos (1-100 permitidos para X e Y): 1d0' });
  });

  test('deve retornar erro para número de lados muito grande (ex: "1d101")', () => {
    const result = rollDiceInternal('1d101');
    expect(result).toEqual({ error: 'Parâmetros de dados inválidos (1-100 permitidos para X e Y): 1d101' });
  });
});
