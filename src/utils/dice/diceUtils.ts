import { type DiceRollDetails } from "../../shared/types/index"; // Ajustar o caminho do tipo

export const rollDiceInternal = (
  notation: string
): DiceRollDetails | { error: string } => {
  // Adicionado ^ e $ para garantir que a string inteira corresponda ao padrão
  const match = notation
    .toLowerCase()
    .match(/^(\d{1,3})d(\d{1,3})(?:([+-])(\d+))?$/);
  ...
  if (numDice <= 0 || diceSides <= 0 || numDice > 100 || diceSides > 100) {
    return {
      error: `Parâmetros de dados inválidos (1-100 permitidos para X e Y): ${notation}`,
    };
  }
