...
export const HealthSection: React.FC<HealthSectionProps> = ({
  ...
  return (
    <fieldset id="pontos de vida" className="flex flex-col space-y-1.5 p-2 rounded-md bg-surface-1 mt-2">
      <legend className="bg-surface-1 p-1 pl-2 pr-3 rounded text-sm font-bold">PONTOS DE VIDA</legend>
      
      {/* SEÇÃO DE HP - AGORA COMPLETA E REGISTRADA */}
      <div>
        <div>
          <label htmlFor="currentHp" className={cn("block text-[0.8rem] font-medium mb-px", "text-[10px] text-center block")}>ATUAL</label>
          <input
            id="currentHp"
            type="number"
        ...
        <div className="flex space-x-1.5 mt-1.5">
          <div className="flex-1">
            <label htmlFor="tempHp" className={cn("block text-[0.8rem] font-medium mb-px", "text-[10px] text-center block")}>TEMPORÁRIO</label>
            <input
              id="tempHp"
              type="number"
          ...
          <div className="flex-1">
            <label htmlFor="maxHp" className={cn("block text-[0.8rem] font-medium mb-px", "text-[10px] text-center block")}>MÁXIMO</label>
            <input
              id="maxHp"
              type="number"
      ...
      <fieldset className="flex-1 rounded bg-surface-1 relative">
        ...
        <button type="button" onClick={handleAddHitDice} className="flex items-center justify-center -bottom-5.5 w-6 h-6 right-[50%] translate-x-1/2 absolute text-xl font-bold bg-accent-primary text-text-primary rounded-[10rem] hover:bg-accent-secondary">
          <PlusCircleIcon />
        </button>
