...
export interface ModalEntry {
  ...
  dismissible?: boolean; // Adicionar propriedade para controlar se o modal pode ser fechado clicando fora
}
...
export const useModalStateManagement = (): UseModalStateManagementReturn => {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const openModal = useCallback((modalName: string, props: Record<string, any> = {}, dismissible: boolean = true) => {
    setModalState(prevState => {
      ...
      const newModalEntry: ModalEntry = {
        ...
        name: modalName,
        props,
        dismissible, // Passar a propriedade dismissible
      };
