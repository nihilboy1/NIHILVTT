...
import { PlayerCharacter, CharacterTypeEnum } from "@/entities/character/model/schemas/character.schema";
...
export function AttributesWidget({ characterId, className }: AttributesWidgetProps) {
  ...
  // Type guard para garantir que estamos lidando com um PlayerCharacter
  if (foundCharacter.type !== CharacterTypeEnum.enum.Player) {
    return <div>Este widget é apenas para Personagens Jogadores.</div>;
  }

  const character: PlayerCharacter = foundCharacter; // Tipagem explícita
  ...
  const handleAttributeChange = (attributeName: keyof PlayerCharacter["attributes"], value: number) => {
  ...
  const handleToggleProficiency = (proficiencyPath: string, isProficient: boolean) => {
