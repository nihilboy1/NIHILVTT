...
describe('ChatCommandPopover', () => {
  ...
  const defaultCommands = [
    { name: '/roll', description: 'Rola dados', usage: '/roll <notation>', aliases: ['/r'], execute: jest.fn() },
    { name: '/whisper', description: 'Envia uma mensagem privada', usage: '/whisper <player> <message>', aliases: ['/w'], execute: jest.fn() },
    { name: '/clear', description: 'Limpa o histórico do chat', usage: '/clear', execute: jest.fn() },
    { name: '/help', description: 'Exibe comandos disponíveis', usage: '/help', execute: jest.fn() },
  ];
  ...
  beforeAll(() => {
    ...
    originalContains = Object.getOwnPropertyDescriptor(HTMLElement.prototype, 'contains');
    ...
    Object.defineProperty(HTMLElement.prototype, 'contains', {
      ...
      value: function(this: HTMLElement, otherNode: Node | null): boolean {
        ...
        return originalContains ? originalContains.value.call(this, otherNode) : false;
      },
    });
  });
  ...
  afterAll(() => {
    ...
    if (originalContains) Object.defineProperty(HTMLElement.prototype, 'contains', originalContains);
    if (originalScrollIntoView) Object.defineProperty(window.HTMLElement.prototype, 'scrollIntoView', originalScrollIntoView);
  ...
  beforeEach(() => {
    ...
    mockContainsImplementation = jest.fn().mockImplementation((elementInstance, nodeArg) => {
    ...
    mockGetBoundingClientRect.mockReturnValue({ 
      top: 100, right: 200, left: 100, bottom: 150, width: 100, height: 50, x: 100, y: 100, toJSON: () => ({}),
    });
  ...
  test('não deve renderizar o popover quando inputValue não começa com /', async () => {
    ...
    const { rerender: initialRerender } = renderComponent({ isOpen: true, inputValue: "/r" });
    ...
    initialRerender(
      <>
        <input ref={targetInputRef} aria-label="Chat Input" />
        <ChatCommandPopover {...defaultProps} isOpen={true} inputValue="hello" />
      </>
    );
  ...
  test('deve renderizar o popover e sugestões quando isOpen é true e inputValue começa com /', async () => {
    ...
    const popover = screen.getByRole('listbox', { name: 'Sugestões de comando' });
  ...
  test('deve exibir "Nenhum comando correspondente." se não houver comandos filtrados', async () => {
    ...
    await waitFor(() => {
      const popover = screen.getByRole('listbox', { name: 'Sugestões de comando' });
      expect(popover).toBeInTheDocument();
      expect(screen.getByText('Nenhum comando correspondente.')).toBeInTheDocument();
  ...
  test('deve selecionar o próximo comando com ArrowDown quando há múltiplos comandos filtrados', async () => {
    ...
    await waitFor(() => expect(getByRole('option', { name: '/roll <notation> Rola dados' })).toHaveAttribute('aria-selected', 'true'));
    await waitFor(() => expect(getByRole('option', { name: '/whisper <player> <message> Envia uma mensagem privada' })).toHaveAttribute('aria-selected', 'false'));

    await userEvent.type(chatInput, '{arrowdown}'); 

    
    await waitFor(() => expect(getByRole('option', { name: '/roll <notation> Rola dados' })).toHaveAttribute('aria-selected', 'false'));
    await waitFor(() => expect(getByRole('option', { name: '/whisper <player> <message> Envia uma mensagem privada' })).toHaveAttribute('aria-selected', 'true'));
  });
  ...
  test('deve manter a seleção (loop) com ArrowDown quando há apenas um comando filtrado', async () => {
    ...
    mockGetAllCommands.mockReturnValue([
        { name: '/roll', description: 'Rola dados', usage: '/roll <notation>', aliases: ['/r'], execute: jest.fn() },
        { name: '/unrelated', description: 'Outro comando', usage: '/unrelated', aliases: [], execute: jest.fn() },
    ]);
  ...
  test('deve selecionar o comando anterior com ArrowUp', async () => {
    ...
    await waitFor(() => expect(screen.getByRole('option', { name: '/roll <notation> Rola dados' })).toHaveAttribute('aria-selected', 'true'));
    await waitFor(() => expect(screen.getByRole('option', { name: '/whisper <player> <message> Envia uma mensagem privada' })).toHaveAttribute('aria-selected', 'false'));

    await userEvent.type(chatInput, '{arrowdown}'); 
    await waitFor(() => expect(screen.getByRole('option', { name: '/roll <notation> Rola dados' })).toHaveAttribute('aria-selected', 'false'));
    await waitFor(() => expect(screen.getByRole('option', { name: '/whisper <player> <message> Envia uma mensagem privada' })).toHaveAttribute('aria-selected', 'true'));
  ...
  test('deve chamar onClose ao clicar fora do popover e do targetInputRef', () => {
    ...
    mockContainsImplementation.mockImplementation((element, otherNode) => {
      ...
      if (otherNode === document.body && (element === popover || element === target)) {
        return false;
      }
      
      return originalContains ? originalContains.value.call(element, otherNode) : false;
    });
  ...
  test('não deve chamar onClose ao clicar no targetInputRef', () => {
    renderComponent({ inputValue: "/r" });
     ...
     mockContainsImplementation.mockImplementation((element, otherNode) => {
      ...
      if (element === targetInputRef.current && otherNode === targetInputRef.current) {
        return true;
      }
