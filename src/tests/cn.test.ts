...
describe('cn', () => {
  ...
  test('deve lidar com classes condicionais corretamente', () => {
    expect(cn('text-red-500', false && 'bg-blue-200', true && 'font-bold')).toBe('text-red-500 font-bold');
  });
  ...
  test('deve lidar com arrays de classes', () => {
    expect(cn(['text-lg', 'font-semibold'], 'text-gray-700')).toBe('text-lg font-semibold text-gray-700');
  });
  ...
  test('deve ignorar valores nulos e indefinidos', () => {
    expect(cn('text-green-500', null, undefined, 'bg-white')).toBe('text-green-500 bg-white');
  });

  test('deve lidar com objetos de classe', () => {
    expect(cn({ 'text-xl': true, 'text-sm': false }, 'font-medium')).toBe('text-xl font-medium');
  });

  test('deve combinar diferentes tipos de entrada', () => {
    expect(cn('p-2', { 'm-4': true }, ['border', 'border-gray-300'])).toBe('p-2 m-4 border border-gray-300');
  });
