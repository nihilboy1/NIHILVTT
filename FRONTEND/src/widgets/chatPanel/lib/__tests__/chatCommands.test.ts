import {
  Command, // Import the Command interface
  CommandContext,
  findCommand,
  getAllCommands,
...
describe("chatCommands", () => {
  ...
  describe("/roll command", () => {
    let rollCommand: Command; // Use the Command type

    beforeAll(() => {
      rollCommand = findCommand("/roll") as Command; // Assert type
    });
    ...
    test("validateArgs deve retornar mensagem de erro para notação inválida", () => {
      ...
      expect(rollCommand.validateArgs?.(["101d6"])).toBe(
        "Dados inválidos. Número de dados (X) e lados (Y) devem ser entre 1 e 100."
      );
      expect(rollCommand.validateArgs?.(["1d0"])).toBe(
        "Dados inválidos. Número de dados (X) e lados (Y) devem ser entre 1 e 100."
      );
  ...
  describe("/help command", () => {
    let helpCommand: Command; // Use the Command type

    beforeAll(() => {
      helpCommand = findCommand("/help") as Command; // Assert type
    });
    ...
    test("execute deve listar todos os comandos quando não há argumentos", () => {
      ...
      expect(message).toContain(
        "/clear - Limpa o histórico do chat. (Alternativas: /cls)"
      );
    });
    ...
    test("execute deve informar quando o comando não é encontrado", () => {
      ...
      expect(mockSendMessage).toHaveBeenCalledWith(
        'Comando "/unknown" não encontrado. Digite /help para ver todos os comandos.',
        "Sistema"
      );
    });
  });
  ...
  describe("/clear command", () => {
    let clearCommand: Command; // Use the Command type

    beforeAll(() => {
      clearCommand = findCommand("/clear") as Command; // Assert type
    });
