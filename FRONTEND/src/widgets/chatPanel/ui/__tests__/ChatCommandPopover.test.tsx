...
import { ChatCommandPopover } from '@/widgets/chatPanel/ui/ChatCommandPopover';

jest.mock('@/widgets/chatPanel/lib/chatCommands');
...
describe('ChatCommandPopover', () => {
  ...
  beforeAll(() => {
    ...
    Object.defineProperty(HTMLElement.prototype, 'contains', {
      ...
      value: function (this: HTMLElement, otherNode: Node | null): boolean {
        ...
        return originalContains?.value.call(this, otherNode) ?? false;
      },
    });
  });
  ...
  afterAll(() => {
    ...
    if (originalContains)
      Object.defineProperty(HTMLElement.prototype, 'contains', originalContains);
  ...
  beforeEach(() => {
    ...
    mockContainsImplementation = jest.fn().mockImplementation((elementInstance, nodeArg) => {
      ...
      // Direct DOM access is necessary here to accurately mock the native `contains` method's behavior.
      // This is an exception to `testing-library/no-node-access` rule.
  ...
  test('deve exibir "Nenhum comando correspondente." se não houver comandos filtrados', async () => {
    ...
    await screen.findByText('Nenhum comando correspondente.');
    await waitFor(() => expect(screen.queryByText('/roll')).not.toBeInTheDocument());
  ...
  test('deve posicionar o popover abaixo do input por padrão', async () => {
    ...
    await waitFor(() => expect(popover).toHaveStyle('top: 158px'));
    await waitFor(() => expect(popover).toHaveStyle('left: 100px'));
  });
  ...
  test('deve selecionar o próximo comando com ArrowDown quando há múltiplos comandos filtrados', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByRole('option', { name: '/roll <notation> Rola dados' })).toHaveAttribute(
        'aria-selected',
        'true',
      ),
    );
