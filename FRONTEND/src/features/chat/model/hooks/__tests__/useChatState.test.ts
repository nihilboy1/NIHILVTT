...
import { DiceRollDetails, DiceRollMessage, TextMessage, RollCategory } from '@/shared/api/types';
...
describe('useChatStore', () => {
  ...
  it('deve inicializar o estado com a mensagem de boas-vindas', () => {
    ...
    expect(result.current.messages[0].text).toContain('Saudações, nobre aventureiro!');
  });
  ...
  it('deve enviar uma mensagem de texto', () => {
    ...
    expect(result.current.messages[1].sender).toBe(DEFAULTS.PLAYER_NAME);
  ...
  it('deve rolar dados e enviar a mensagem de resultado', () => {
    ...
    expect(sentMessage.sender).toBe(DEFAULTS.PLAYER_NAME);
    expect(sentMessage.isDiceRoll).toBe(true);
