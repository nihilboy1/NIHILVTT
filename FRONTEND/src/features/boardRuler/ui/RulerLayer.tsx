...
export function RulerLayer({ rulerPath, zoomLevel, gridSettings }: RulerLayerProps) {
  ...
  // O resto do código JSX permanece exatamente o mesmo, pois ele já tem os dados que precisa.
    ...
    <g>
      {rulerPath.points.map((wp: RulerPointData, index: number) => {
        ...
        if (showTextCondition) {
          ...
          const textY = wp.point.y - rulerWaypointRadius - textBgHeight / 2 - 2 / zoomLevel;
          waypointTextElement = (
