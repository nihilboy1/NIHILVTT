...
import { parseTokenSize } from '../utils/tokenUtils';
    ...
    const handleMouseUp = (event: MouseEvent) => {
      ...
      if (isDragging) {
        if (currentVisualPosition) {
          const [sizeMultiplierX, sizeMultiplierY] = parseTokenSize(characterSize);
          let finalGridX = Math.round(currentVisualPosition.x / cellSize);
          let finalGridY = Math.round(currentVisualPosition.y / cellSize);

          finalGridX = Math.max(
            0,
            Math.min(finalGridX, pageSettings.widthInUnits - Math.ceil(sizeMultiplierX)),
          );
          finalGridY = Math.max(
            0,
            Math.min(finalGridY, pageSettings.heightInUnits - Math.ceil(sizeMultiplierY)),
          );
