...
export type MonsterManifestSpeed = {
  walk: number | null;
  burrow: number | null;
  climb: number | null;
  fly: number | null;
...
export type MonsterManifestAbilityScores = {
  strength: number;
  dexterity: number;
  constitution: number;
  intelligence: number;
...
export type MonsterManifestDefenses = {
  resistances: string[];
  vulnerabilities: string[];
  damageImmunities: string[];
};

export type MonsterManifestConditionalDamage = {
  damageFormula: string;
  damageType: string;
  requiresUserMovementAtLeastMeters: number;
};

export type MonsterManifestConditionalCondition = {
  condition: string;
  requiresUserMovementAtLeastMeters: number;
};

export type MonsterManifestAction = {
  catalogAttackId: string;
  actionId: string;
  name: string;
  attackBonus: number;
  damageFormula: string;
...
export type MonsterManifestPassiveEffect = {
...
export type MonsterCatalogManifestEntry = {
  id: string;
  primaryName: string;
  names: string[];
  tokenUrl: string;
...
export type MonsterCatalogManifest = {
  manifestVersion: number;
  combatContractVersion: number;
  monsters: MonsterCatalogManifestEntry[];
};

export const MONSTER_CATALOG_MANIFEST_VERSION = 1;
export const MONSTER_COMBAT_CONTRACT_VERSION = 1;
...
function convertDistanceToMeters(normal: number, unit: unknown): number {
  ...
  throw new Error("Unidade de distância inválida no catálogo de monstros.");
}
...
function buildMonsterAutomatedPassives(
  monster: (typeof PHB2024MONSTERS)[number],
  attackActions: MonsterManifestAction[],
): MonsterManifestPassiveEffect[] {
  return monster.effects
    .flatMap((effect) => {
      if (
        effect.type !== "passive_grantAdvantage" ||
        effect.on !== "attackRoll"
      ) {
        ...
        ? [
            ...new Set(
              effect.appliesToActions
                ...
                .flatMap((actionId) => {
                  const mappedCatalogAttackIds = attackActions
                    .filter((attackAction) => attackAction.actionId === actionId)
                    .map((attackAction) => attackAction.catalogAttackId);
                  if (mappedCatalogAttackIds.length === 0) {
                    throw new Error(
                      `passive_grantAdvantage com appliesToActions inválido: ${monster.id}:${effect.name}:${actionId}`,
                    );
                  }

                  return mappedCatalogAttackIds;
                }),
            ),
          ].sort((left, right) => left.localeCompare(right))
        : [];
    ...
    .sort((left, right) => {
      ...
      return (
        left.triggerHasAllyNearby.rangeMeters -
        right.triggerHasAllyNearby.rangeMeters
      );
    });
}
...
function buildMonsterAttackActions(
  ...
  const seenCatalogAttackIds = new Set<string>();
  ...
  return monster.effects
    .flatMap((effect) => {
      ...
      const hasAttackType =
        Array.isArray(parameters.attackType) && parameters.attackType.length > 0;
      const hasHitDamageOutcome = outcomes.some(
        (outcome) =>
          Boolean(outcome) &&
      ...
      const isCanonicalAttackAction =
        actionId === "act-attack" &&
        (hasAttackType ||
          hasHitDamageOutcome ||
          typeof attackBonusValue === "number");
      if (!isCanonicalAttackAction) {
        return [] as MonsterManifestAction[];
      }
      ...
      const baseDamageOutcome = firstDamageOutcome ?? fallbackFirstDamageOutcome;

      if (!baseDamageOutcome) {
        throw new Error(
          `Monster attack action sem outcome de dano base (modifyTargetHP on hit): ${monster.id}:${actionId}:${effect.name}`,
        );
      }

      if (typeof attackBonusValue !== "number") {
        throw new Error(
          `Monster attack action sem attackBonus válido: ${monster.id}:${actionId}:${effect.name}`,
        );
      }

      if (typeof rangeNormal !== "number" || rangeNormal <= 0) {
        throw new Error(
          `Monster attack action sem range.normal válido: ${monster.id}:${actionId}:${effect.name}`,
        );
      }

      if (typeof rangeUnit !== "string" || rangeUnit.trim().length === 0) {
        throw new Error(
          `Monster attack action sem range.unit válido: ${monster.id}:${actionId}:${effect.name}`,
        );
      }
      ...
      const conditionalDamageBonuses = outcomes
        .flatMap((outcome) => {
          ...
          const conditionalDamageType = extractDamageType(outcomeRecord.formula);
      ...
      const attackNameSlug = slugify(effect.name);
      if (attackNameSlug.length === 0) {
        throw new Error(
          `Monster attack action sem nome canônico utilizável em ID: ${monster.id}:${actionId}`,
        );
      }

      const catalogAttackId = `${actionId}:${attackNameSlug}`;
      if (seenCatalogAttackIds.has(catalogAttackId)) {
        throw new Error(
          `Monster attack action com catalogAttackId duplicado: ${monster.id}:${catalogAttackId}`,
        );
      }
      seenCatalogAttackIds.add(catalogAttackId);
...
export function buildMonsterCatalogManifest(): MonsterCatalogManifest {
  return {
    manifestVersion: MONSTER_CATALOG_MANIFEST_VERSION,
    combatContractVersion: MONSTER_COMBAT_CONTRACT_VERSION,
    monsters: [...PHB2024MONSTERS]
      .map((monster) => {
        const actions = buildMonsterAttackActions(monster);
        return {
          id: monster.id,
          ...
          actions,
          automatedPassives: buildMonsterAutomatedPassives(monster, actions),
        };
      })
